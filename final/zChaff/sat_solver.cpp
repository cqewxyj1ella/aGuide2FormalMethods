...
// function: read cnf from file and construct the problemd
void read_cnf(SAT_Manager mng, char* filename)
{
	...
	set<int> clause_vars; 
	// set's insert will omit same elements: thus polarities (which naturally satisfied) 
	// will be ommited to the clauses
	set<int> clause_lits;
	// this set is to judge polarites: if <lits>.size != <vars>.size
	int line_num = 0;
		...
		cl_idx = SAT_GetNextClause(mng, cl_idx))
	{
		...
		for (i = 0; i < len; ++i)
		{
			...
			if ((var_value == 1 && sign == 0) ||
				(var_value == 0 && sign == 1))
				break; // this clause is SAT
		}
		if (i >= len) // all the clause are not SAT
		{
...
int main(int argc, char** argv)
{
	...
	if (argc < 2) {
		...
		cerr << "Copyright 2000-2004, Princeton University." << endl
			<< endl;
		;
	...
	if (argc == 2) {
		read_begin = clock();
		read_cnf(mng, argv[1]); // read cnf from file and construct the problemd
		read_end = clock();
	}
	else {
		read_cnf(mng, argv[1]);
	...
	int result = SAT_Solve(mng); // main part of solver
