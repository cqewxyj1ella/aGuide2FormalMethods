...
void CSolver::init_solve(void)
{
	...
	assert(_dlevel == 0);
	// it seems to be related to VSIDS, init counters
...
void CSolver::set_var_value_current_dl(int v, int value)
{
	...
	for (vector<CLitPoolElement*>::iterator itr = watchs.begin(); itr != watchs.end(); ++itr)
	{
		...
		while (1)
		{
			...
			if (ptr->is_watched())
			{ // literal is the other watched lit, skip it.
				other_watched = ptr;
				continue;
			}
...
bool CSolver::decide_next_branch(void)
{
	...
	// below: choose a var according to score
	...
	for (unsigned i = 0; i < _ordered_vars.size(); ++i)
		cout
		<< (_ordered_vars[i].first - &variables()[0]) << " ";
	cout << endl;);
	...
	for (unsigned i = 0; i < _ordered_vars.size(); ++i) {
		...
		assert(i == 0 || _ordered_vars[i].second <= _ordered_vars[i - 1].second);
		assert(_ordered_vars[i].first->value() != UNKNOWN || _max_score_pos <= i);
		if (_ordered_vars[i].first->value() == UNKNOWN)
			++free;
...
int CSolver::preprocess(void)
{
	...
	for (i = 1, sz = variables().size(); i < sz; ++i)
	{
		...
		{ // both pos and neg literal appear nonce
			un_used.push_back(i);
	...
	if (_params.verbosity > 1 && un_used.size() > 0) // verbosity is a switch for printf info
	{
	...
	for (i = 1, sz = variables().size(); i < sz; ++i)
	{
		...
		if (v.value() != UNKNOWN)
			continue;
		if (v.lits_count(0) == 0)
		{ // no positive phased lits.
			queue_implication(i + i + 1, NULL_CLAUSE, 0); // i+i+1 = index<<1+sign(1)
			uni_phased.push_back(-i);
		}
		else if (v.lits_count(1) == 0)
		{ // no negative phased lits.
			queue_implication(i + i, NULL_CLAUSE, 0); // i+i = index<<1+sign(0)
			uni_phased.push_back(i);
		}
	}
		...
		if (decide_next_branch())
		{
			while (deduce() == CONFLICT)
			{
				...
				for (int step_i = 0; step_i < max_steps; step_i++) {
					...
					vector<int> conflict_copy;
					conflict_copy.assign(_conflicts.begin(), _conflicts.end());
					/*int shortest = conflict_copy.back();
					int len = (int)clause(shortest).num_lits();
					// choose the var to flip
					int flip_index = 0;
					int max_count = 0;
					for (int i = 0; i < len; i++) {
						auto literal_i = clause(shortest).literal(i);
						int index = literal_i.var_index();
						int sign = literal_i.var_sign();
						auto var = variable(index);
						if ((var.value() == (unsigned)(sign & 0x1)) && (var.lits_count(sign) > max_count)) {
							max_count = var.lits_count(sign);
							flip_index = index;
						}
					}*/
					int flip_index;
					for (int ass_i = assignments.size(); ass_i >=0; --ass_i) {
					...
					// now flip
					changed_var_index.push_back(flip_index);
					int assigned = assignments[flip_index];
					int svar = assigned ^ 0x1;
					variable(flip_index).set_new_cl_phase((assigned ^ 0x1) & 0x1);
					//variable(svar >> 1).set_value(svar & 0x1);
				...
				int blevel = -1;
				if (SLS_flag == false) {// SLS failed, backtrach on changed var
					...
					while(!changed_var_index.empty()) {
						...
						//variable(index).set_value(!variable(index).value());
					}
				}
				else {
					blevel = 1;
...
void CSolver::mark_vars_at_level(ClauseIdx cl, int var_idx, int dl)
{
	...
	for (CLitPoolElement* itr = clause(cl).literals(); (*itr).val() > 0; ++itr)
	{
		...
		{
			assert(variable(v).dlevel() < dl);
			if (variable(v).new_cl_phase() == UNKNOWN)
			{ // it's not in the new cl
...
int CSolver::analyze_conflicts(void)
{
	...
	if (dlevel() == 0)
	{ // already at level 0. Conflict means unsat.
#ifdef VERIFY_ON
		for (unsigned i = 1; i < variables().size(); ++i)
		{
			if (variable(i).value() != UNKNOWN)
...
int CSolver::conflict_analysis_lastUIP(void)
{
	...
	{ // now add conflict lits, and unassign vars
		...
		if (variable(assigned >> 1).is_marked())
		{
			...
			{ // last UIP is the decision varialbe
				assert(i == 0);
...
void CSolver::update_var_score(void)
{
	...
	for (int i = 0; i < _ordered_vars.size(); ++i)
		cout
		<< (_ordered_vars[i].first - &variables()[0]) << "(" << _ordered_vars[i].first->score() << ") ";
	cout << endl;);
...
int CSolver::add_clause_incr(int* lits, int num_lits, int gid)
{
	...
	for (i = 0, sz = clause(cl).num_lits(); i < sz; ++i)
	{
		...
		if (value == UNKNOWN)
			continue;
		if (variable(var_idx).dlevel() == 0 && variable(var_idx).antecedent() == -1 && literal_value(clause(cl).literal(i)) == 0)
		{
			back_track(0);
			if (preprocess() == CONFLICT)
				_stats.outcome = UNSATISFIABLE;
