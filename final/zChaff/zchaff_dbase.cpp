...
ClauseIdx CDatabase::add_clause(int * lits, int n_lits, int gflag)  { 
    ...
    while (lit_pool_free_space() <= n_lits + 1) { 
        if (enlarge_lit_pool()==false) // FIXME: didn't figure out enlarge's necessity and thesis
    ...
    if(n_lits==2) {
        ...
        // count array may say how important is this var
    ...
    if (cl.num_lits() > 1) {
        ...
        for (i=0; i< sz; ++i) {
            ...
            if (literal_value(cl.literal(i)) != 0 ){ // FIXME: don't know what is watch list
                v.watched(v_sign).push_back( & cl.literal(i));
                cl.literal(i).set_watch(1);
                break; // why break after add to watch list?
            }
            else {
                if (v.dlevel() > max_dl) {
                    max_dl = v.dlevel();
